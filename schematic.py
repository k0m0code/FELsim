    ...
    def plotBeamPositionTransform(self, matrixVariables, beamSegments, interval, defineLim = True, saveData = False, shape = {}, plot = True):
        ...
        for i in range(len(beamSegments)):
            ...
            while intTrack >= interval:
                ...
                if defineLim:
                    maxVals, minVals = self.checkMinMax(matrixVariables, maxVals, minVals)
                tempNewXVal = x_axis[-1]
                plot6dValues.update({tempNewXVal: (ebeam.getXYZ(matrixVariables))})
                if tempNewXVal%1 == 0:
                    xLabelIndice.append(len(x_axis) - 1)
                intTrack -= interval
            if intTrack > 0:
        ...
        if plot:
            ...
            xLabels = []
            xLabelsSet = set(xLabelIndice)
            for xi in range(len(x_axis)):
                if xi in xLabelsSet:
                    xLabels.append(str(x_axis[xi]))
                else:
                    xLabels.append("")
